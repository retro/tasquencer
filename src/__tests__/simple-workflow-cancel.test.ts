...
it('handles workflow cancellation in simple workflows (1)', () => {
  const program = Effect.gen(function* ($) {
    ...
    expect(state4.workflows[0]?.state).toEqual('canceled');
  });

  Effect.runSync(program);
});

it('handles workflow failure in simple workflows (2)', () => {
  const program = Effect.gen(function* ($) {
    const idGenerator = makeIdGenerator();
    ...
    expect(getEnabledTaskNames(state4)).toEqual(new Set());
    expect(state5.workflows[0]?.state).toEqual('canceled');
  });
