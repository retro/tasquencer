...
it('runs a net with "or" split and "or" join (with using explicit condition for default or split target)', ({
  expect,
}) => {
  const workflowDefinition = Builder.workflow<{
    ...
    .connectTask('register', (to) =>
      to
        ...
        .defaultCondition('register->book_hotel')
    )
    .connectCondition('register->book_hotel', (to) => to.task('book_hotel'))
  ...
  const program = Effect.gen(function* () {
    ...
    const service3 = yield* workflowDefinition.build().pipe(
      Effect.flatMap((workflow) =>
        Service.initialize(workflow, {
    ...
    yield* service3.startRootWorkflow();
